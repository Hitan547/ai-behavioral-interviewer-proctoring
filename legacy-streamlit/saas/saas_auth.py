...
def show_saas_login_signup():
    ...
    Also handles password reset via URL query params.
    """
    init_saas_db()
    init_db()

    # Check for password reset flow via URL params
    params = st.query_params
    reset_token = params.get("reset_token", "")
    if reset_token:
        show_password_reset_form(reset_token)
        return

    # Check if user is on the forgot password page
...
def show_candidate_login():
    ...
    if st.button("Forgot Password?", key="candidate_forgot_pw", type="secondary"):
...
def show_recruiter_login():
    ...
    if st.button("Forgot Password?", key="recruiter_forgot_pw", type="secondary"):
        st.session_state._show_forgot_password = True
        st.rerun()
...
def show_forgot_password_page():
    ...
    with col:
        ...
        <div style="text-align:center;margin-bottom:24px">
          <div style="font-size:22px;font-weight:800;color:var(--text)">Reset Password</div>
        ...
        if submitted:
            if not email or not validate_email(email):
                ...
                from password_reset import generate_reset_token, send_reset_email_via_n8n
                result = generate_reset_token(email.strip())

                if result.get("success") and result.get("token"):
                    app_url = os.getenv("APP_BASE_URL", "http://localhost:8501")
                    reset_url = f"{app_url}?reset_token={result['token']}"
                    sent = send_reset_email_via_n8n(
                        email=email.strip(),
                        reset_url=reset_url,
                        display_name=result.get("display_name", "User"),
                    )
                    if sent:
                        st.success("✅ Reset link sent! Check your email inbox.")
                    else:
                        # Show the link directly if email sending fails
                        st.warning("Email delivery is not configured. Use this link to reset:")
                        st.code(reset_url)
                else:
...
def show_password_reset_form(token: str):
    """Password reset form — user sets new password using the token from email."""
    from password_reset import verify_reset_token, execute_password_reset
    ...
    with col:
        ...
        if not check["valid"]:
            ...
            <div style="text-align:center;margin-bottom:24px">
              ...
              <div style="font-size:18px;font-weight:700;color:#991b1b">Invalid Reset Link</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style="text-align:center;margin-bottom:24px">
          <div style="font-size:22px;font-weight:800;color:var(--text)">Set New Password</div>
          <div style="font-size:13px;color:var(--muted);margin-top:5px">
            Resetting password for <strong>{check['email']}</strong>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if submitted:
            if not new_pw or len(new_pw) < 6:
                ...
                result = execute_password_reset(token, new_pw)
                if result["success"]:
